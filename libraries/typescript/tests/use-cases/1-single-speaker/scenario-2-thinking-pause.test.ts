...
describe('Use Case 1: Single Speaker — Scenario 2 (Thinking Pause)', () => {
  const simulator = new LivePhoneCallSimulator(16000);

  it('asserts TurnSense stays incomplete during pause, turns complete after continuation', async () => {
    const rawPcm = simulator.generateCallStream({ scenario: 2, numSpeakers: 1, noiseLevelDbfs: -50 });
