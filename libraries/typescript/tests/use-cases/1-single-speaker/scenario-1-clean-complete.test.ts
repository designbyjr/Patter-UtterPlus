...
describe('Use Case 1: Single Speaker — Scenario 1 (Clean Complete Turn)', () => {
  const simulator = new LivePhoneCallSimulator(16000);

  it('asserts VAD start -> VAD end -> TurnSense complete -> Policy dispatch', async () => {
    const rawPcm = simulator.generateCallStream({ scenario: 1, numSpeakers: 1, noiseLevelDbfs: -50 });
    const vad = await TenVAD.load({ minSpeechDuration: 0.1, minSilenceDuration: 0.1 });
    const turnSense = await TurnSenseDetector.load();

    const events: string[] = [];
    const wsFrameSize = 640; // 20ms WebSocket PCM frame @ 16kHz
