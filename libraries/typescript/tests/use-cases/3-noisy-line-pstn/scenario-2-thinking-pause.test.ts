...
describe('Use Case 3: Noisy Line PSTN — Scenario 2 (Thinking Pause)', () => {
  const simulator = new LivePhoneCallSimulator(16000);

  it('asserts thinking pause holds open turn on noisy PSTN line', async () => {
    const rawPcm = simulator.generateCallStream({ scenario: 2, numSpeakers: 1, noiseLevelDbfs: -38 });
