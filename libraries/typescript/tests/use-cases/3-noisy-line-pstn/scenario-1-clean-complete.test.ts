...
describe('Use Case 3: Noisy Line PSTN — Scenario 1 (Clean Complete Turn)', () => {
  const simulator = new LivePhoneCallSimulator(16000);

  it('asserts clean turn completion on noisy PSTN line (-38 dBFS background noise)', async () => {
    const rawPcm = simulator.generateCallStream({ scenario: 1, numSpeakers: 1, noiseLevelDbfs: -38 });
