...
15. **Independent Temporal Deployment & Container Isolation.** The Temporal Server & UI infrastructure is 100% decoupled from `Patter-UtterPlus`. It maintains its own dedicated GitHub repository (`designbyjr/Patter-Temporal`), located locally in the project directory at `./temporal/` (git-ignored from `Patter-UtterPlus`), its own dedicated `wrangler.toml` file (`./temporal/wrangler.toml`), its own independent GitHub Actions CI/CD deployment pipeline, and executes in its own separate isolated container/Worker instance (`patter-temporal-service`). Never route Temporal UI or gRPC traffic through the `Patter-UtterPlus` main Worker (`patter-voice-agent`) or parent `wrangler.toml`. **Modifying or discussing Temporal MUST NEVER alter `src/worker/index.ts` or any files in the parent `Patter-UtterPlus` repository.** Whenever Temporal is discussed or modified, AI agents MUST explicitly ask the user which repository (`Patter-UtterPlus` SDK client vs `Patter-Temporal` container/server) the task targets.
